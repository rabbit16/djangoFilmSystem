...
class Captcha(object):
    ...
    def initialize(self, width=200, height=75, color=None, text=None, fonts=None, number=4):
        # self.image = Image.new('RGB', (width, height), (255, 255, 255))
        self._text = text if text else random.sample(string.ascii_uppercase + string.ascii_uppercase + '3456789', number)
...
# captcha.initialize(number=6)
if __name__ == '__main__':
    num_letters = eval(input("请输入你想要验证码多长: "))
    captcha.initialize(number=6)
    content = captcha.generate_captcha()
    loop = eval(input("你想要生成几张验证码："))
    while loop>=0:
        with open("{}.jpg".format(content[0]), 'wb') as f:
            f.write(content[1])
        loop -= 1
        captcha.initialize(number=6)
        content = captcha.generate_captcha()
